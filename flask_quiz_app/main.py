...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///quiz.db'  # Veritabanı URL'sini buraya yazın
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)
migrate = Migrate(app, db)
from app.models import Score, Question, Answer 
...
def result():
    ...
    conn = get_db_connection()
    best_score_data = conn.execute(
        "SELECT MAX(score) AS best_score FROM scores WHERE username = ?",
        (session["username"],),
    ).fetchone()

    best_score = best_score_data["best_score"] if best_score_data else 0  # Eğer veri yoksa 0 ata.
